...
To load the module within a python session (e.g. using the python3/3.10.0 environment we used to compile and install `dynlib` library) we can use the following:
{% highlight python %} 
...
{% endhighlight %} 
- It assumes that functions exist in some of the source files. (e.g. `lib/settings.py` doesn't contain a function `get_active_context`, although it's discussed in a comment!)  Also, the requirement for `pygrib` isn't in the documentation), e.g.
{% highlight python %} 
